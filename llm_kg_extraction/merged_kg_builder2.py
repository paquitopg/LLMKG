...
class FinancialKGBuilder:
    ...
    def _process_page_parallel(self, page_info: Dict) -> Dict:
        """
        Process a single page in parallel mode with its own LLM client.
        
        Args:
            page_info (Dict): Dictionary containing page number and page data.
        ...
        local_llm = self._create_llm_client()
        
        print(f"Worker processing page {page_num + 1}...")
        
        # For text-only extraction
        if self.extraction_mode == "text":
            # Create a temporary instance with the local LLM
            temp_instance = type(self)(
                model_name=self.model_name,
                deployment_name=self.deployment_name,
                project_name=self.project_name,
                construction_mode="onego",  # Each page is processed independently
                extraction_mode="text"
            )
            temp_instance.llm = local_llm
            
            # Analyze text
            result = temp_instance.analyze_text_with_llm(page_data["text"])
        ...
        # For multimodal extraction
            ...
            # Analyze page with visual elements
            visual_analysis = temp_instance.identify_visual_elements(page_data)
            visual_kg = temp_instance.extract_data_from_visuals(page_data, visual_analysis)
            text_kg = temp_instance.analyze_text_with_llm(page_data["text"])
    ...
    def _build_knowledge_graph_parallel(self, dump: bool = False) -> Dict:
        ...
        Each page is processed independently by a separate worker with its own LLM client.
        ...
        # Prepare input data for each worker
        page_inputs = []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_page = {
                executor.submit(self._process_page_parallel, page_input): page_input["page_num"]
        ...
        page_kgs = [result["graph"] for result in all_page_results]
