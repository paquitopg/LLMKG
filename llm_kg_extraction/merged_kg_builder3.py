...
class FinancialKGBuilder:
    ...
    def build_prompt_for_text_analysis(self, text: str, previous_graph: Dict = None) -> str:
        ...
        3.  Identify and create relationships between entities, based *only* on information in the "Current Page Text to Analyze".
            - Prioritize relationships linking to the Target Company "{target_company_name}" or other significant entities *if supported by the current page's text*.
            - Ensure relationships are created if entities are clearly linked in the text.
        ...
        6.  Normalize name variations for the same entity based on the "Current Page Text to Analyze". If an entity is also in the "Context from PREVIOUS Pages", prefer the established name/ID for consistency if appropriate for the current mention.
        7.  Assign unique IDs (e.g., "e1", "e2") to each *new* entity found on the *current page*. If an entity mentioned in the "Current Page Text to Analyze" is identical to one in the "Context from PREVIOUS Pages", reuse its existing ID.
        8.  Be accurate and comprehensive based on the current page text. Ensure the JSON is valid.
